...
class LowRankPolynomialClassifierSplittingStrategy(SplittingStrategy):
    def __init__(self, k= 2 ,prettify=True, determinizer=LabelPowersetDeterminizer(), linear_svc_params={}):
        """ Args:
    ...
    def find_split(self, dataset, impurity_measure, **kwargs):
        ...
        for label in labels:
            ...
            #(Alan+Giacomo): For each potential polynomial split, the following two lines make the low rank approximation.  
            A=split.get_coef_matrix() #(Alan+Giacomo): This function returns the symmetric coefficient matrix A of the polynomial found by the above svc. It can be found in the "LowRankPolynomialSplit" class below.
            split.k_rank_approx(A) #(Alan+Giacomo): This function can be found in the "LowRankPolynomialSplit" class below.
        ...
        imp, bestSplit, label_mask, standardizer, svc = min(
            splits, key=lambda t: t[0])   #Choosing the best based on impurity measure
...
class LowRankPolynomialSplit(Split, ABC):
    ...
    def __init__(self, pipeline, coefs, numeric_columns, priority=1):
        ...
        self.F=None #(Alan+Giacomo): This attribute will store the eigenvector matrix F (sorted in the order of the eigenvalues and truncated after k-1 columns, where k is found by the largest_gap function) which we will use to write the sum of squares.
        self.eigenvalues=None #(Alan+Giacomo): This attribute will store the first k-1 sorted eigenvalues, where k is found by the largest_gap function below.
    ...
    def get_equation_str(self, rounded=False, newlines=False, variables=None, machineReadable=False): #change string to sum of squares
        ...
        print("length of eigenvalues: ", len(self.eigenvalues))
        print("length of variables: ", len(variables))
        for i in range(len(self.eigenvalues)):
            inside=[]
            inside.append(f"{self.eigenvalues[i]}(")
            print("F matrix: ", self.F)
            for j in range(len(variables)):
                if variables[j]=="1":
                    inner=f"{self.F[j][i]}"
                else:
                    inner=f"{self.F[j][i]}*{variables[j]}"
                inside.append(inner)
            joiner = "\\n+" if newlines else "+"
    ...
    def k_rank_approx(self, A):
        eigvals, F = np.linalg.eig(A) 
        sorted_indexes=np.argsort(eigvals)[::-1] #(Alan+Giacomo): This sorts the eigenvalues in decreasing order
        eigvals = eigvals[sorted_indexes]
        F = F[:,sorted_indexes]
        k=self.relative_largest_gap(eigvals) #(Alan+Giacomo): Finds the index of the largest relative gap in the eigenvalues. Function found below.
        #Problem: if we select more than one k, then we have to do something different here
        eigvals[k:]=0
        D=np.diag(eigvals)
        A=F@D@(F.T) #(Alan+Giacomo): Here we compute the low rank approximation.
        self.F=F[:,:k]
        self.eigenvalues=eigvals[:k]
    ...
    def relative_largest_gap(self, L):
        #(Alan+Giacomo): This function computes the index of the relative largest gap in the eigenvalues using ratios.
        current=0
        k=len(L)
        for i in range(len(L)-1):
            if L[i+1]/L[i]>current:
                current=L[i]/L[i+1] #Problem, there's a warning saying that we divide by 0 sometimes.
                k=i+1
        return k
        #add function for multiple largest gaps
