...
class LowRankPolynomialClassifierSplittingStrategy(SplittingStrategy):
    ...
    def find_split(self, dataset, impurity_measure, **kwargs):
        ...
        for label in labels:
            ...
            original_split=copy(split) #(Alan+Giacomo): Here we store a copy of the original split in a new variable so that we can apply multiple values of k to approximate it
            #(Alan+Giacomo): For each potential polynomial split, the following two lines make the low rank approximation.  
            #split.get_coef_matrix() #(Alan+Giacomo): This function returns the symmetric coefficient matrix A of the polynomial found by the above svc. It can be found in the "LowRankPolynomialSplit" class below.
            gaps=split.relative_largest_gaps() #(Alan+Giacomo): This function can be found in the "LowRankPolynomialSplit" class below.
            
            for k in gaps:
                split=copy(original_split)
                split.k_rank_approx(k)
...
class LowRankPolynomialSplit(Split, ABC):
    ...
    def get_coef_matrix(self):
        ...
        sorted_indexes=np.absolute(eigvals).argsort()[::-1] #(Alan+Giacomo): This sorts the eigenvalues in decreasing order
        return A, eigvals[sorted_indexes], F[:,sorted_indexes]
    
    def k_rank_approx(self, k):       
    ...
    def relative_largest_gaps(self):
        ...
        L=self.eigenvalues
        print(f"Eigenvalues {'  '.join(map(str, L))}")
        ratios = [abs(L[i] / L[i - 1]) for i in range(1, len(L))]
        print(f"Ratios {'  '.join(map(str, ratios))}")
        k = [i for i in range(1, len(L)) if abs(L[i]) < 0.1 and abs(L[i] / L[i - 1]) < 0.1] + [len(L)]
        print(f"k is {k}")
        return k
