...
class LowRankPolynomialClassifierSplittingStrategy(SplittingStrategy):
    def __init__(self,prettify=True, z=0.1, determinizer=LabelPowersetDeterminizer(), linear_svc_params={}):
        """ Args:
    ...
    def find_split(self, dataset, impurity_measure, **kwargs):
        ...
        for label in labels:
            ...
            gaps=split.relative_largest_gaps(self.z) #(Alan+Giacomo): This function can be found in the "LowRankPolynomialSplit" class below.
...
class LowRankPolynomialSplit(Split, ABC):
    ...
    def prettify_update(self):
        A, eigenvalues, F = get_coef_matrix()
        self.F=F[:,:self.k]
        self.eigenvalues=eigenvalues[:self.k]


    def relative_largest_gaps(self, z):
        #(Alan+Giacomo): This function now returns a list with all the viable values of k.
        L=self.eigenvalues
        if 0 in L:
            indices = [i for i in range(1, L.index(0)+1) if abs(L[i]) < 0.1 and abs(L[i] / L[i - 1]) < z]
        else:
            indices = [i for i in range(1, len(L)) if abs(L[i]) < 0.1 and abs(L[i] / L[i - 1]) < z] #z is set to 0.1 by default
        if len(indices)==0:
            indices=[len(L)]
        return indices
