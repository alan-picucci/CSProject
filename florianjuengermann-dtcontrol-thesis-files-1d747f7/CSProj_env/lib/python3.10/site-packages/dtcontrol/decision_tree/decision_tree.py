...
from dtcontrol.decision_tree.splitting.polynomial_lowrank import LowRankPolynomialClassifierSplittingStrategy, LowRankPolynomialSplit
...
class DecisionTree(BenchmarkSuiteClassifier):
    def __init__(self, splitting_strategies, impurity_measure, name, label_pre_processor=None, early_stopping=False,
        ...
        self.check_valid()
        self.low_rank=any(isinstance(strat, LowRankPolynomialClassifierSplittingStrategy) for strat in self.splitting_strategies)
    ...
    def get_stats(self):
        if any(isinstance(strat, LowRankPolynomialClassifierSplittingStrategy) for strat in self.splitting_strategies):
            return {
                ...
                'k=1': self.root.k[0], #(Alan+Giacomo): saving the number of times that a split is made by a polynomial approximated by each value of k.
                'k=2': self.root.k[1],
                 'k=3': self.root.k[2],
                 'k=4': self.root.k[3]
            }
...
class Node:
    ...
    def fit(self, dataset, **kwargs):
        ...
        splits = [strategy.find_split(dataset, self.impurity_measure, **kwargs) for strategy in self.splitting_strategies] #(Alan+Giacomo): this finds the best splits for each splitting strategy included (for example, axis aligned and lowrank_polynomial) so it can then compare their impurities to determine which is better. This function leads to "polynomial_lowrank.py" when we use our new splitting strategy.
        ...
        if any(isinstance(strat, LowRankPolynomialClassifierSplittingStrategy) for strat in self.splitting_strategies):
            for i in range(4):
                if self.split.k==i+1:
                    self.k[i]=1+sum([c.k[i] for c in self.children])
                else:
                    self.k[i]=sum([c.k[i] for c in self.children])
